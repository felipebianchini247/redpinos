...
import { saveContactoAction } from '../actions';
import type { ContactoContent } from '@/lib/content';

const empty: ContactoContent = { whatsapp: '', whatsappNota: '', email: '', direccion: '' };
...
export default function AdminContactoPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setPending(true);
    setMessage(null);
    const result = await saveContactoAction(new FormData(e.currentTarget));
    ...
    <AdminLayout title="Editar Contacto">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <input
            name="whatsappNota"
            value={form.whatsappNota}
            onChange={(e) => setForm({ ...form, whatsappNota: e.target.value })}
          />
        </div>
        <div className="form-group">
          <label>Email</label>
